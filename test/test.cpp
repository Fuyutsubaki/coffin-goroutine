...
struct SuspendAlways {
  ...
  void await_suspend(std::coroutine_handle<> h) { cfn::detail::ready_park(h); }
  void await_resume() {}
};
...
IUTEST(Goroutine, subtask_return) {
  ...
  auto g = std::make_shared<cfn::Goroutine>(
      [&]() -> cfn::Task<> { n = *(co_await gen_subtask()); }());
  g->execute();
  IUTEST_ASSERT_EQ(42, n);
}
...
struct MyScheduler {
  ...
  void stop() { on_ready.store(false); }
...
template <class T> MyChannel<T> make_channel(std::size_t queue_size) {
...
IUTEST(BasicChannel, Select) {
  ...
  gsc.spown_task([&]() -> cfn::Task<> {
    while (1) {
      auto [done, ret] = co_await cfn::select(done_ch.recv(), ch.recv());
      if (done) {
        break;
      } else if (ret) {
        tmp.push_back(**ret);
...
IUTEST(BasicChannel, SenderRecver1) {
  std::vector<int> tmp;
  {
    ...
    for (int i = 0; i < 10; ++i) {
      gsc.spown_task([](auto sender) -> cfn::Task<> {
        for (int i = 0; i < 10; ++i) {
    ...
    gsc.spown_task([&](auto recver) -> cfn::Task<> {
      for (;;) {
...
template <class value_type>
using MyChannel2 =
    cfn::BasicChannel<BoostThreadpoolSchedulerWrapper, value_type>;

IUTEST(WithBoostAsio, SenderRecver1) {
  std::vector<int> tmp;
  {
    auto [sender, recver] =
        cfn::makeChannel<BoostThreadpoolSchedulerWrapper, int>(
            BoostThreadpoolSchedulerWrapper{gbts}, 0);
    for (int i = 0; i < 10; ++i) {
      gbts.spown_task([](auto sender) -> cfn::Task<> {
        for (int i = 0; i < 10; ++i) {
          co_await sender->send(i);
        }
      }(sender));
    ...
    gbts.spown_task([&](auto recver) -> cfn::Task<> {
      for (;;) {
        auto r = co_await recver->recv();
        if (!r)
          break;
...
IUTEST(Example, C100k) {
  std::vector<int> tmp;
  {
    ...
    for (int i = 0; i < 100000; ++i) {
      auto [new_sender, new_recver] =
          cfn::makeChannel<BoostThreadpoolSchedulerWrapper, int>(
              BoostThreadpoolSchedulerWrapper{gbts}, 0);
      gbts.spown_task([](auto recver, auto sender) -> cfn::Task<> {
        for (;;) {
          auto r = co_await recver->recv();
          if (!r)
            break;
  ...
  gbts.run(4);
  std::vector<int> expected = {100000, 100001, 100002, 100003, 100004,
...
struct Context {
  ...
  MyChannel<std::monostate> ch = make_channel<std::monostate>(0);
  auto done() { return ch.recv(); }
  void cancel() {
    bool expected = false;
...
IUTEST(Example, ContextSample) {
  ...
  int i = 0;
  std::vector<int> tmp;
  gbts.spown_task([&i](auto &ctx, auto sender) -> cfn::Task<> {
    for (; i < 10000; ++i) {
      auto [sendret, done] = co_await cfn::select(sender->send(i), ctx.done());
      if (done) {
        break;
      }
    }
  }(ctx, sender));
  gbts.spown_task([&tmp](auto &ctx, auto recver) -> cfn::Task<> {
    for (int i = 0; i < 100; ++i) {
      std::optional<int> x = co_await recver->recv();
      tmp.push_back(*x);
    }
    ctx.cancel();
  }(ctx, recver));
  gbts.run(4);
  IUTEST_ASSERT_EQ(i, 100);
}
